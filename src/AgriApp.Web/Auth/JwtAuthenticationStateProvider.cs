...
public class JwtAuthenticationStateProvider : AuthenticationStateProvider
{
    ...
    {
        // On first call during boot, return Anonymous immediately to avoid blocking JS Interop initialization.
        // The token will be checked on subsequent navigation or when explicitly needed.
        if (!_initialized)
        {
            _initialized = true;
            // Return Anonymous synchronously to allow Blazor WASM boot to complete.
            // JS Interop is now safe to use on subsequent calls.
            return Anonymous;
        }
    ...
    /// <summary>
    /// Manually refresh the authentication state from LocalStorage.
    /// Call this after the app has fully booted to load the persisted token.
    /// </summary>
