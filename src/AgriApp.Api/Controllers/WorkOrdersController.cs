...
public class WorkOrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        return workOrder == null ? NotFound(new { error = "Work order not found" }) : Ok(workOrder);
    }
    ...
    public async Task<IActionResult> Create([FromBody] CreateWorkOrderRequest request)
    {
        ...
        var workOrder = await _service.CreateAsync(
            request.EquipmentId, request.StaffId, request.Description, centerId);

        return Created($"/api/work-orders/{workOrder.Id}", workOrder);
    }

    [HttpPatch("{id:int}/status")]
    [Authorize(Roles = "SuperUser,Manager,Supervisor,Staff")]
    public async Task<IActionResult> UpdateStatus(int id, [FromBody] UpdateWorkOrderStatusRequest request)
    {
        if (!Enum.TryParse<WorkStatus>(request.Status, out var status))
            return BadRequest(new { error = "Invalid status. Must be Pending, InProgress, Completed, or Cancelled" });
